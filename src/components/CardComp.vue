  ...
  <div id="box">
    <div class="mt-5 pt-5 px-2 w-100 container" id="searchBar">
      <input
        ...
        placeholder="Search by categoies or product names"
        aria-label="Search"
        v-model="search"
        @change="searchByName()"
      />
      <div class="dropdown">
        ...
        <ul class="dropdown-menu">
          ...
          <li>
            <a class="dropdown-item" @click="HightToLow()">High to low</a>
          </li>
        </ul>
      </div>
      <div>
        ...
        <ul class="dropdown-menu">
          ...
          <li>
            <a class="dropdown-item" @click="sortByNameDesc">Descending</a>
          </li>
        </ul>
      </div>
    </div>
  ...
  methods: {
    ...
    searchByName() {
      ...
      let resultY = storageArr.filter((prod) => {
        return (
          prod.product_name.toLowerCase().includes(inputX.toLowerCase()) ||
          prod.product_category.toLowerCase().includes(inputX.toLowerCase())
        );
      });
      return resultY;
    },
...
#productImg:hover {
  ...
  border-image: fill 0
    linear-gradient(rgb(249, 249, 249), rgb(140, 79, 215), rgb(249, 249, 249));
  transition: all 1s ease-in-out;
}
